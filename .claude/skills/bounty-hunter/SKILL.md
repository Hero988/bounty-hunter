...
- CSWSH: Must prove the WebSocket actually processes authenticated commands from evil origin, not just that the 101 handshake completes. **Pre-check: Identify which cookies are AUTHENTICATION cookies vs CDN/WAF cookies (e.g., `__cf_bm` is Cloudflare bot management, NOT an auth cookie). Verify the SameSite attribute of the AUTHENTICATION cookies specifically. If auth cookies are `SameSite=Lax` (browser default since 2020) or `SameSite=Strict`, CSWSH is blocked by browser protections regardless of missing origin validation — do NOT report.** Only if auth cookies are confirmed `SameSite=None` AND you can demonstrate authenticated data access from an evil origin in a real browser PoC should you proceed. **(X/xAI 2026-03: CSWSH report on code.grok.com closed as Informative — report proved 101 handshake but confused Cloudflare `__cf_bm` SameSite=None with SSO auth cookies. Triager ruled browser protections prevent exploitation.)**
